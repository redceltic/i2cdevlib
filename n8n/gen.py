...
"    params: {\n"
"      stundensatz_eur: 200,       // Fallback-Stundensatz; ein BELEGTER Quellwert (Transkript) hat Vorrang\n"
...
"      mandate_pro_monat: 3,       // Volumen-Standardannahme - treibt den Nutzen linear\n"
"      entwicklungsstunden: 24,    // Einmaliger Aufbauaufwand (Std.) - Backstop; belegte Quellangabe (z.B. 'zwei Tage') hat Vorrang\n"
...
"const herkunft = [];\n"
"function pick(obj, key, def, label, floor) {\n"
...
"const sats   = pick(n03,'stundensatz_eur', p.stundensatz_eur, 'Stundensatz EUR/h');\n"
...
"const vol    = pick(n03,'mandate_pro_monat', p.mandate_pro_monat, 'Vorgaenge/Monat', 1);\n"
...
"const preis_belegt = (Number.isFinite(preisNum) && (preisArt==='belegt' || preisArt==='geschaetzt')) ? preisNum : null;\n"
...
"let wertmodell;\n"
"if (preis_belegt != null) wertmodell = 'umsatz_neu';\n"
"else if (wird_erbracht && ausgelastet) wertmodell = 'effizienz';\n"
"else wertmodell = 'effizienz_unsicher';\n"
...
"herkunft.push({ groesse:'Abrechenbarer Zeitanteil af', wert: af_real, quelle: af_quelle, herleitung: 'AUSLASTUNGS-ANNAHME, keine Tatsache. Band ' + af_low + '/' + af_real + '/' + af_high + '. Fakten: wird_erbracht=' + wird_erbracht + ', nachfrage=' + nachfrage + ', preis_belegt=' + (preis_belegt!=null) + '.' });\n"
"// Kostentreiber\n"
"const ent_h  = pick(n04,'entwicklungsstunden', p.entwicklungsstunden, 'Entwicklungsstunden', 8);\n"
...
"const nutzen_bedingt = (umsatz_potenzial_m > 0) || (wertmodell === 'effizienz_unsicher');\n"
...
"const belegtN = herkunft.filter(x => /belegt/i.test(x.quelle)).length;\n"
"const fallbacks = herkunft.filter(x => /Config-Standard|Config-Band|Mindestwert|Unbekannt/i.test(x.quelle)).length;\n"
"const validiert = !!p.validiert;\n"
"let konfidenz = Math.max(0.2, Math.round((0.35 + 0.55 * (belegtN / herkunft.length)) * 100) / 100);\n"
"if (!validiert) konfidenz = Math.min(konfidenz, 0.6);\n"
"const status_zahlen = validiert ? 'final' : 'vorlaeufig';\n"
"const warnungen = [];\n"
"if (nutzen_bedingt) warnungen.push('VERKAUFSNUTZEN BEDINGT: Umsatzmodell ohne belegten Verkaufspreis/gesicherte Nachfrage - das Verkaufspotenzial ist NICHT beziffert (vor Entscheidung klaeren). Der ausgewiesene realisierte Nutzen enthaelt nur den Opportunitaetswert der gesparten Zeit (Anteil af), nicht den Verkauf.');\n"
"if (ausl_belegt == null && wertmodell !== 'umsatz_neu') warnungen.push('Abrechenbarer Zeitanteil (af) NICHT belegt - Auslastungs-Annahme als Band ' + af_low + '/' + af_real + '/' + af_high + ' angesetzt; bestimmt den realisierten Effizienznutzen stark. Bei freier Kapazitaet ist af real eher klein. In Config.params (af_szenarien) pruefen.');\n"
"if (wertmodell === 'effizienz_unsicher') warnungen.push('EFFIZIENZ UNSICHER: Taetigkeit wird heute nicht (belegt) erbracht oder Kapazitaet ist nicht ausgelastet - gesparte Zeit fuehrt nur dann zu Geld, wenn sie nachweislich fuer bezahlte Arbeit genutzt wird. Realisierter Nutzen daher als Spanne (worst ' + Math.round(worst_m*12) + ' bis best ' + Math.round(best_m*12) + ' EUR/Jahr).');\n"
...
"    + 'Brutto-Zeitwert (Potenzial bei af=1) = ' + brutto_zeitwert_jahr + ' EUR/Jahr (NICHT realisiert).' + (umsatz_potenzial_m>0 ? ' Zusaetzliches Verkaufspotenzial bei gesicherter Nachfrage = ' + Math.round(umsatz_potenzial_m*12) + ' EUR/Jahr (NICHT realisiert).' : '') + ' '\n"
...
"wertmodell:'Wertmodell', nachfrage_gedeckt:'Nachfrage gesichert', nutzen_bedingt:'Nutzen bedingt', af_band:'Abrechenbarer Zeitanteil af (Band)', konservativ:'Konservativ', optimistisch:'Optimistisch', abrechenbarkeit_belegt:'Auslastung belegt', vorgaenge_pro_monat:'Vorgaenge/Monat', vorgaenge_monat:'Vorgaenge/Monat', brutto_zeitwert_eur_jahr:'Brutto-Zeitwert \\u20ac/Jahr (Potenzial)', potenzial_eur_jahr:'Gesamtpotenzial \\u20ac/Jahr (nicht realisiert)', realisiert_spanne_eur_jahr:'Realisierter Nutzen \\u20ac/Jahr (Spanne)', sensitivitaet_af:'Sensitivitaet (abrechenbarer Zeitanteil)', af:'af', break_even_monate:'Break-Even (Monate)' };\n"
...
"  '<h2>Kennzahlen' + vBadge + '</h2><table><tr><td style=\"padding:6px 12px;border-bottom:1px solid #eee\">Realisierter Nutzen (Jahr) <span style=\"color:#888;font-weight:normal\">&middot; real</span></td><td style=\"padding:6px 12px;border-bottom:1px solid #eee;text-align:right\"><strong>' + num(n05.nutzen_eur_jahr,' \\u20ac') + '</strong>' + ((n05.realisiert_spanne_eur_jahr) ? '<br><span style=\"font-size:11px;color:#888\">Spanne ' + num(n05.realisiert_spanne_eur_jahr.worst,' \\u20ac') + ' &ndash; ' + num(n05.realisiert_spanne_eur_jahr.best,' \\u20ac') + '</span>' : '') + '</td></tr>' +\n"
"    '<tr><td style=\"padding:6px 12px;border-bottom:1px solid #eee;color:#888\">Brutto-Zeitwert (Potenzial bei voller Auslastung, nicht realisiert)</td><td style=\"padding:6px 12px;border-bottom:1px solid #eee;text-align:right;color:#888\">' + num(n05.brutto_zeitwert_eur_jahr,' \\u20ac') + '</td></tr>' +\n"
"    ((n05.potenzial_eur_jahr && n05.potenzial_eur_jahr > n05.nutzen_eur_jahr) ? '<tr><td style=\"padding:6px 12px;border-bottom:1px solid #eee;color:#888\">Gesamtpotenzial inkl. Verkauf (nicht realisiert)</td><td style=\"padding:6px 12px;border-bottom:1px solid #eee;text-align:right;color:#888\">' + num(n05.potenzial_eur_jahr,' \\u20ac') + '</td></tr>' : '') +\n"
